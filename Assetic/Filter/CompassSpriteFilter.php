 ...
 */
class CompassSpriteFilter extends BaseProcessFilter implements DependencyExtractorInterface
{
	private $compassPath;
	private $rubyPath;
	private $scss;
	...
	protected function getBundleDirectory($pathMask, $currentPath)
	{
		$path = $pathMask;
		...
		if (strpos($path, '{bundle}') !== false) {
			...
			$bundleName = strtolower($matches[1] . $matches[2]);

			$path = str_replace('{bundle}', $bundleName, $path);
		}
	...
	public function filterLoad(AssetInterface $asset)
	{
		...
		if ($this->imagesDir) {
			$pb->add('--images-dir')->add($this->getBundleDirectory($this->imagesDir, $root));
		}
		...
		if ($this->httpImagesPath) {
			$optionsConfig['http_images_path'] = $this->getBundleDirectory($this->httpImagesPath, $root);
		}
		...
		if ($this->httpGeneratedImagesPath) {
			$optionsConfig['http_generated_images_path'] = $this->getBundleDirectory($this->httpGeneratedImagesPath, $root);
		}

		if ($this->generatedImagesPath) {
			$optionsConfig['generated_images_path'] = $this->getBundleDirectory($this->generatedImagesPath, $root);
		}
